    ...
    <div class="relative p-4 w-full max-w-xl max-h-full">
        <div class="relative bg-white rounded-lg shadow-sm">
            ...
            <div class="p-4 md:p-5">
                <form action="{{ route('verifikasi-permohonan.update', $data->first()->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="max-w-screen-xl mx-auto">
                        <div class="w-full bg-white border border-gray-200 rounded-lg shadow p-3">
                            ...
                            <div class="mt-4">
                                <label for="status" class="block py-2 text-sm font-medium">Status Permohonan</label>
                                <select id="status" name="status"
                                    class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    {{ $data->first()->status == 'Disetujui' ? 'disabled' : '' }}>
                                    <option selected disabled>--- Pilih Status ---</option>
                                    <option value="Disetujui"
                                        {{ $data->first()->status == 'Disetujui' ? 'selected' : '' }}>Disetujui
                                    </option>
                                    <option value="Ditolak"
                                        {{ $data->first()->status == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                                </select>
                            </div>
                            <button type="submit"
                                class="mt-4 rounded-lg bg-blue-600 px-4 py-2.5 text-white transition-colors hover:bg-blue-700
                                {{ $data->first()->status == 'Disetujui' ? 'opacity-50 cursor-not-allowed' : '' }}"
                                {{ $data->first()->status == 'Disetujui' ? 'disabled' : '' }}>
                                Simpan
                            </button>
                        </div>
