    ...
    <div class="relative p-4 w-full max-w-lg max-h-full">
        <div class="relative bg-white rounded-lg shadow-sm">
            ...
            <div class="p-4 md:p-5">
                <form action="{{ route('satuan.update', $data->id) }}" method="POST" class="space-y-3">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="nama_satuan" class="block mb-2 text-sm font-medium text-gray-600">Nama
                            Satuan</label>
                        <input type="text" name="nama_satuan" id="nama_satuan" value="{{ $data->nama_satuan }}"
