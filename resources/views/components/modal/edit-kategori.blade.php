    ...
    <div class="relative p-4 w-full max-w-lg max-h-full">
        <div class="relative bg-white rounded-lg shadow-sm">
            ...
            <div class="p-4 md:p-5">
                <form action="{{ route('kategori.update', $data->id) }}" method="POST" class="space-y-3">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="nama_kategori" class="block mb-2 text-sm font-medium text-gray-600">Nama
                            Kategori</label>
                        <input type="text" name="nama_kategori" id="nama_kategori" value="{{ $data->nama_kategori }}"
